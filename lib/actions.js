...
export async function tamuAction(prev, formData) {
  const nama = String(formData.get('nama') || '').trim().slice(0, 100);
  const umurRaw = String(formData.get('umur') || '').trim();
  const alamat = String(formData.get('alamat') || '').trim().slice(0, 255);
  if (!nama) return { pesan: 'Nama wajib diisi.' };
  let umur = null;
  if (umurRaw !== '') {
    umur = parseInt(umurRaw, 10);
    if (!Number.isInteger(umur) || umur < 5 || umur > 120) {
      return { pesan: 'Umur tidak valid (5–120).' };
    }
  }
  await setSession({ id: null, username: 'tamu', nama, role: 'tamu', tamu: true, umur, alamat: alamat || null });
  redirect('/');
}
...
export async function pulihkanTamu(data) {
  const nama = String(data?.nama || '').trim().slice(0, 100);
  if (!nama) return { ok: false };
  let umur = Number.isInteger(data?.umur) ? data.umur : parseInt(data?.umur, 10);
  if (!Number.isInteger(umur) || umur < 5 || umur > 120) umur = null;
  const alamat = String(data?.alamat || '').trim().slice(0, 255) || null;
  await setSession({ id: null, username: 'tamu', nama, role: 'tamu', tamu: true, umur, alamat });
  return { ok: true };
}
...
export async function simpanLevel(prev, formData) {
  await needAdmin();
  const id = String(formData.get('id') || '');
  const kode = String(formData.get('kode') || '').trim().toLowerCase().replace(/[^a-z0-9-]/g, '');
  const nama = String(formData.get('nama') || '').trim();
  const menus = ['psikotest', 'belajar', 'latihan', 'admin'].filter((m) => formData.get('menu_' + m));
  if (!kode || !nama) return { pesan: 'Kode dan nama level wajib diisi.' };
  if (menus.length === 0) return { pesan: 'Pilih minimal satu hak akses menu.' };
  const sql = db();
  if (id) {
    const cur = await sql`SELECT kode FROM roles WHERE id = ${Number(id)}`;
    if (cur.length === 0) return { pesan: 'Level tidak ditemukan.' };
    if (cur[0].kode === 'admin' && !menus.includes('admin')) {
      return { pesan: 'Level admin wajib memiliki hak akses admin.' };
    }
    await sql`UPDATE roles SET nama = ${nama}, menus = ${menus} WHERE id = ${Number(id)}`;
  } else {
    const cek = await sql`SELECT id FROM roles WHERE kode = ${kode}`;
    if (cek.length > 0) return { pesan: 'Kode level sudah terpakai.' };
    await sql`INSERT INTO roles (kode, nama, menus) VALUES (${kode}, ${nama}, ${menus})`;
  }
  const { bustRoleCache } = await import('./roles.js');
  bustRoleCache();
  revalidatePath('/admin/level');
  redirect('/admin/level');
}

export async function hapusLevel(formData) {
  await needAdmin();
  const sql = db();
  const cur = await sql`SELECT kode FROM roles WHERE id = ${Number(formData.get('id'))}`;
  if (cur.length === 0) redirect('/admin/level');
  if (cur[0].kode === 'admin') throw new Error('Level admin tidak boleh dihapus.');
  const pakai = await sql`SELECT COUNT(*)::int AS n FROM users WHERE role = ${cur[0].kode}`;
  if (pakai[0].n > 0) throw new Error(`Level masih dipakai ${pakai[0].n} user. Pindahkan dulu user-nya.`);
  await sql`DELETE FROM roles WHERE id = ${Number(formData.get('id'))}`;
  const { bustRoleCache } = await import('./roles.js');
  bustRoleCache();
  revalidatePath('/admin/level');
  redirect('/admin/level');
...
export async function tambahStaff(prev, formData) {
  await needAdmin();
  const username = String(formData.get('username') || '').trim();
  const nama = String(formData.get('nama') || '').trim();
  const password = String(formData.get('password') || '');
  const kode = String(formData.get('role') || 'manager');
  if (!username || !nama || !password) return { pesan: 'Username, nama, dan password wajib diisi.' };
  if (password.length < 6) return { pesan: 'Password minimal 6 karakter.' };
  const sql = db();
  const role = await sql`SELECT kode FROM roles WHERE kode = ${kode}`;
  if (role.length === 0) return { pesan: 'Level tidak dikenal.' };
  const cek = await sql`SELECT id FROM users WHERE LOWER(username) = LOWER(${username})`;
  if (cek.length > 0) return { pesan: 'Username sudah terpakai.' };
  await sql`INSERT INTO users (username, password, nama, role)
    VALUES (${username}, ${bcrypt.hashSync(password, 10)}, ${nama}, ${kode})`;
...
export async function ubahRole(formData) {
  await needAdmin();
  const sql = db();
  const kode = String(formData.get('role') || 'user');
  const cek = await sql`SELECT kode FROM roles WHERE kode = ${kode}`;
  const role = cek.length > 0 ? kode : 'user';
